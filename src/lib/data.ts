...
export const getAllCountries = () => {
  return Country.getAllCountries().map(country => ({ name: country.name, isoCode: country.isoCode }));
};

export const getStatesOfCountry = (countryIsoCode: string) => {
  return State.getStatesOfCountry(countryIsoCode).map(state => ({ name: state.name, isoCode: state.isoCode }));
};
